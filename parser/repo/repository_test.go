...
type insertSuite struct {
	baseSuite
	parsedTxs []parser.ParsedTx
	poolInfos []parser.PoolInfo
	pairs     []parser.Pair
	height    uint64
}
...
func (s *insertSuite) SetupSuite() {
	...
	s.pairs = parser.FakeParserPairs()
	pairLen := len(s.pairs)
	parsedTxs := parser.FakeParserParsedTxs()
	for ; len(parsedTxs) < pairLen; parsedTxs = parser.FakeParserParsedTxs() {
	}
	poolInfos := parser.FakeParserPoolInfoTxs()
	for ; len(poolInfos) < pairLen; poolInfos = parser.FakeParserPoolInfoTxs() {
	}
