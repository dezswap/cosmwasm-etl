...
func New(repo dex.PairRepo, logger logging.Logger, c configs.ParserDexConfig) (dex.TargetApp, error) {
	finder, err := dexterra.CreateCreatePairRuleFinder(c.FactoryAddress)
	if err != nil {
	...
	if err != nil {
		return nil, errors.Wrap(err, "classicv2.New")
	}
	...
	return &appImpl{repo, &parsers, dex.DexMixin{}, pairs, lpPairAddrs, make(map[string]bool)}, nil
}
...
func (p *appImpl) ParseTxs(tx parser.RawTx, height uint64) ([]dex.ParsedTx, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "classicv2.ParseTxs create_pair tx_hash=%s", tx.Hash)
	}
	...
	if len(createPairTxs) > 0 {
		if err := p.updatePairScopedParsers(); err != nil {
			return nil, errors.Wrapf(err, "classicv2.ParseTxs refresh_pair_parsers tx_hash=%s", tx.Hash)
		}
	}
	...
	for _, raw := range tx.LogResults {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "classicv2.ParseTxs pair_action tx_hash=%s", tx.Hash)
		}
		...
		if p.HasProvide(ptxs) {
			...
			if err != nil {
				return nil, errors.Wrapf(err, "classicv2.ParseTxs initial_provide tx_hash=%s", tx.Hash)
			}
		...
		if err != nil {
			wrapped := errors.Wrapf(err, "classicv2.ParseTxs wasm_transfer tx_hash=%s", tx.Hash)
		...
		if err != nil {
			return nil, errors.Wrapf(err, "classicv2.ParseTxs tax_payment tx_hash=%s", tx.Hash)
		}
		...
		if raw.Type == eventlog.TransferType {
			...
			if err != nil {
				return nil, errors.Wrapf(err, "classicv2.ParseTxs sort_transfer_attrs tx_hash=%s", tx.Hash)
			}
		...
		if err != nil {
			return nil, errors.Wrapf(err, "classicv2.ParseTxs transfer tx_hash=%s", tx.Hash)
		}
		...
		if err != nil {
			return nil, errors.Wrapf(err, "classicv2.ParseTxs burn tx_hash=%s", tx.Hash)
		}
...
func (p *appImpl) getPairAddrs(pairTxs []*dex.ParsedTx) []string {
...
func (p *appImpl) getPairTransfers(pairAddrs []string, transferTxs []*dex.ParsedTx) []*dex.ParsedTx {
...
func (p *appImpl) deductTaxFromPairTxs(taxTxs, pairTransferTxs, pairTxs []*dex.ParsedTx) []*dex.ParsedTx {
...
func (p *appImpl) extractTaxTransfers(transferTxs, taxTxs []*dex.ParsedTx, pairAddrs []string) (taxTransfers, remaining []*dex.ParsedTx) {
	if len(taxTxs) == 0 {
...
func (p *appImpl) UpdateParsers(tokenExceptions map[string]bool, height uint64) error {
	...
	{
		burnRule, err := classicv2.CreateBurnRuleFinder()
		if err != nil {
