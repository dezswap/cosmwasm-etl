...
func New(repo dex.PairRepo, logger logging.Logger, c configs.ParserDexConfig) (dex.TargetApp, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "dexterra.New")
	}
...
func (p *appImpl) ParseTxs(tx parser.RawTx, height uint64) ([]dex.ParsedTx, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "dexterra.ParseTxs create_pair tx_hash=%s", tx.Hash)
	}
	...
	for _, raw := range tx.LogResults {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "dexterra.ParseTxs pair_action tx_hash=%s", tx.Hash)
		}
		...
		if p.HasProvide(ptxs) {
			...
			if err != nil {
				return nil, errors.Wrapf(err, "dexterra.ParseTxs initial_provide tx_hash=%s", tx.Hash)
			}
		...
		if err != nil {
			wrapped := errors.Wrapf(err, "dexterra.ParseTxs wasm_transfer tx_hash=%s", tx.Hash)
		...
		if raw.Type == eventlog.TransferType {
			...
			if err != nil {
				return nil, errors.Wrapf(err, "dexterra.ParseTxs sort_transfer_attrs tx_hash=%s", tx.Hash)
			}
		...
		if err != nil {
			return nil, errors.Wrapf(err, "dexterra.ParseTxs burn tx_hash=%s", tx.Hash)
		}
