...
func Test_parseTxs(t *testing.T) {
	...
	const (
		factoryAddrKey = string(terra.ClassicV1Factory)
		height         = uint(100)
	)
	...
	setUp := func(tc testcase) dex.DexParserApp {
		...
		app := appImpl{&repo, &dex.PairParsers{CreatePairParser: &createPairParser}, dex.DexMixin{}, pairMap, make(map[string]bool)}
