...
type mapper interface {
	toParsedTxModel(chainId string, height uint64, tx dex.ParsedTx) schemas.ParsedTx
	toPoolInfoModel(chainId string, height uint64, pool dex.PoolInfo) schemas.PoolInfo
	toPairModel(chainId string, pair dex.Pair) schemas.Pair

	toPairDto(pair schemas.Pair) dex.Pair
}
...
func (*parserMapperImpl) toPairModel(chainId string, pair dex.Pair) schemas.Pair {
	return schemas.Pair{
		ChainId:  chainId,
...
func (*parserMapperImpl) toPairDto(pair schemas.Pair) dex.Pair {
	return dex.Pair{
...
func (p *parserMapperImpl) toParsedTxModel(chainId string, height uint64, tx dex.ParsedTx) schemas.ParsedTx {
	lpAmount := p.emptyStringToZero(tx.LpAmount)
	if tx.Type == dex.Withdraw {
		lpAmount = "-" + lpAmount
	}
...
func (*parserMapperImpl) toPoolInfoModel(chainId string, height uint64, pool dex.PoolInfo) schemas.PoolInfo {
