...
func Test_TransferMapper(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			&parser.ParsedTx{"", time.Time{}, parser.Transfer, pair.ContractAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], "-1000"}, {pair.Assets[1], ""}}, "", "", "", map[string]interface{}{"recipient": userAddr}},
			"",
		},
		{
			&transferMapper{pairSet: pairSet},
			...
			&parser.ParsedTx{"", time.Time{}, parser.Transfer, userAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], "1000"}, {pair.Assets[1], ""}}, "", "", "", map[string]interface{}{"recipient": pair.ContractAddr}},
			"",
		},
		{
			&transferMapper{pairSet: pairSet},
			...
			&parser.ParsedTx{"", time.Time{}, parser.Transfer, userAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], "1000"}, {pair.Assets[1], "2000"}}, "", "", "", map[string]interface{}{"recipient": pair.ContractAddr}},
			"",
		},
		{
			&transferMapper{pairSet: pairSet},
			...
			&parser.ParsedTx{"", time.Time{}, parser.Transfer, userAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], ""}, {pair.Assets[1], "123456789012345678901234567890123456"}}, "", "", "", map[string]interface{}{"recipient": pair.ContractAddr}},
			"",
		},
		{
			&transferMapper{pairSet: pairSet},
...
func Test_CreatePairMapper(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			&parser.ParsedTx{"", time.Time{}, parser.CreatePair, "", "A", []parser.Asset{{"xpla1xumzh893lfa7ak5qvpwmnle5m5xp47t3suwwa9s0ydqa8d8s5faqn6x7al", ""}, {"axpla", ""}}, "xpla1gte4eejaw3hrs2d8pt0zhp0yfd34xp24qdgqumjul29jt5hwl5tsx3qmw7", "", "", nil},
			"",
		},
		{
			&createPairMapper{},
...
func Test_PairMapper(t *testing.T) {
	...
	tcs := []struct {
...

		{
			...
			&parser.ParsedTx{"", time.Time{}, parser.Swap, userAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], "100000"}, {pair.Assets[1], "-100583"}}, "", "", "302", nil},
			"",
		},
		{
			...
			&parser.ParsedTx{"", time.Time{}, parser.Swap, userAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], "-100583"}, {pair.Assets[1], "100000"}}, "", "", "300", nil},
			"",
		},
		{
			&pairMapperImpl{&pairMapperMixin{pairSet: pairSet}},
		...
		/// Provide
		{
			...
			&parser.ParsedTx{"", time.Time{}, parser.Provide, userAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], "1000"}, {pair.Assets[1], "10000"}}, pair.LpAddr, "998735", "", nil},
			"",
		},
		{
			...
			&parser.ParsedTx{"", time.Time{}, parser.Provide, userAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], "1000"}, {pair.Assets[1], "10000"}}, pair.LpAddr, "998735", "",
				map[string]interface{}{"refund_assets": []parser.Asset{{pair.Assets[0], "0"}, {pair.Assets[1], "100"}}}},
			"",
		},
		{
			&pairMapperImpl{&pairMapperMixin{pairSet: pairSet}},
		...
		/// Withdraw
		{
			...
			&parser.ParsedTx{"", time.Time{}, parser.Withdraw, userAddr, pair.ContractAddr, []parser.Asset{{pair.Assets[0], "-24999998"}, {pair.Assets[1], "-24939789"}}, pair.LpAddr, "12418119", "", nil},
			"",
		},
		{
			&pairMapperImpl{&pairMapperMixin{pairSet: pairSet}},
...
func Test_InitialProvideMapper(t *testing.T) {
	...
	tcs := []struct {
		...
		/// Initial Provide
		{
			...
			&parser.ParsedTx{"", time.Time{}, parser.InitialProvide, "", pair.ContractAddr, []parser.Asset{}, pair.LpAddr, "1000", "", nil},
			"",
		},
		{
...
func Test_PairV2ProvideMapper_applyRefund(t *testing.T) {
	tcs := []struct {
		provide  []parser.Asset
		refund   []parser.Asset
		expected []parser.Asset
		errMsg   string
	}{
		{
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "1000"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "10000"}},
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "0"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "100"}},
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "1000"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "9900"}},
			"",
		},
		{
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "1234567890"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "10000"}},
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "123456"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "9999"}},
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "1234567890"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "1"}},
			"",
		},
		{
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "1000"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "10000"}},
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "0"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "9999"}},
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "1000"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "10"}},
			"Asset2 must be 1",
		},
		{
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "1000"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "10000"}},
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "10"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "9999"}},
			[]parser.Asset{{xpla.NATIVE_GOVERNANCE_TOKEN, "990"}, {fmt.Sprintf("%s%s", xpla.CW20_PREFIX, "Asset2"), "1"}},
